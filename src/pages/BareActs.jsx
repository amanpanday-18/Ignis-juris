            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div className="text-center mb-12 relative">
                    <div className="flex items-center justify-center mb-4">
                        ...
                        <h1 className="text-4xl font-bold text-primary">Bare Acts Database</h1>
                    </div>
                ...
                {loading ? (
                    ...
                    <div className="grid grid-cols-1 gap-6">
                        {acts.map((act, index) => (
                            <motion.div
                                ...
                                className="bg-white rounded-xl shadow-md border border-gray-200 p-6 hover:shadow-lg transition-shadow relative group"
                            >
                                {/* Admin Delete Button */}
                                {isAdmin && (
                                    <button
                                ...
                                <div className="flex flex-col md:flex-row justify-between items-start md:items-center">
                                    <div className="flex-1">
                                        <div className="flex items-center mb-2">
                                            <span className="px-3 py-1 bg-blue-50 text-blue-700 rounded-full text-xs font-bold uppercase tracking-wider mr-3">
                                        ...
                                        <h3 className="text-xl font-bold text-primary mb-2">{act.title}</h3>
                                        {act.act_number && (
                                            <p className="text-sm text-gray-500 font-medium mb-2">{act.act_number}</p>
                                        )}
                                        <p className="text-gray-600 mb-4 line-clamp-2">{act.description}</p>
                                    </div>

                                    <div className="mt-4 md:mt-0 md:ml-6 flex-shrink-0">
                                        {act.content_url ? (
                                            <a
                                                href={act.content_url}
                                                target="_blank"
                                                rel="noopener noreferrer"
                                                className="flex items-center px-4 py-2 bg-gray-100 text-gray-700 font-medium rounded-lg hover:bg-accent hover:text-white transition-colors"
                                            >
                                                <FileText className="h-4 w-4 mr-2" />
                                                View / Download
                                            </a>
                                        ) : (
                                            <span className="text-gray-400 text-sm italic">No document available</span>
                                        )}
                                    </div>
                                </div>
                            </motion.div>
                    ...
                    <div className="text-center py-12">
                        <p className="text-gray-500 text-lg">No acts found matching your search.</p>
                    </div>
                )}
            </div>
