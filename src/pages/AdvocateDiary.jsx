        ...
        return (
            <div className="min-h-screen flex items-center justify-center bg-gray-50">
                <div className="text-center">
                    ...
                    <p className="text-gray-600">You need to be logged in to access your diary.</p>
                </div>
            </div>
        );
    }
                                    ...
                                    return (
                                        <div
                                            ...
                                            className={`h-24 p-2 rounded-lg border cursor-pointer transition-all hover:shadow-md flex flex-col ${isSelected
                                                    ? 'border-accent bg-accent/5 ring-2 ring-accent ring-opacity-50'
                                                    : isToday
                                                        ? 'border-blue-300 bg-blue-50'
                                                        : 'border-gray-100 bg-white hover:border-gray-300'
                                                }`}
                                        >
                                            <span className={`text-sm font-bold mb-1 ${isToday ? 'text-blue-600' : 'text-gray-700'}`}>
                                                {day}
                                            </span>
                                            <div className="flex-1 overflow-y-auto space-y-1 no-scrollbar">
                                                {dayEvents.map(event => (
                                                    <div
                                                        key={event.id}
                                                        className={`text-[10px] px-1 py-0.5 rounded truncate ${eventTypes.find(t => t.id === event.event_type)?.color || 'bg-gray-100'
                                                            }`}
                                                    >
                                                        {event.title}
                                                    </div>
                                ...
                                {loading ? (
                                    ...
                                    selectedDateEvents.map(event => (
                                        <motion.div
                                            ...
                                            className={`p-4 rounded-lg border-l-4 shadow-sm relative group ${eventTypes.find(t => t.id === event.event_type)?.color.replace('text-', 'bg-opacity-10 border-') || 'border-gray-300 bg-gray-50'
                                                }`}
                                        >
                                            <button
